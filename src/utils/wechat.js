const { get, getJson, getText, post } = require('./request');

async function getUUID() {
    const timestamp = Date.now();
    const jsloginUrl = `https://login.wx.qq.com/jslogin?appid=wx782c26e4c19acffb&redirect_uri=https%3A%2F%2Fwx.qq.com%2Fcgi-bin%2Fmmwebwx-bin%2Fwebwxnewloginpage&fun=new&lang=zh_CN&_=${timestamp}`;
    const jsloginText = await getText(jsloginUrl);
    const uuid = /uuid\s?=\s?"(\S+)"/g.exec(jsloginText)[1];
    return uuid;
}

async function cheakLoginInfo(uuid) {
    const timestamp = Date.now();
    const loginUrl = `https://login.wx.qq.com/cgi-bin/mmwebwx-bin/login?loginicon=true&uuid=${uuid}&tip=0&r=${~timestamp}&_=${timestamp}`;

    const loginInfoText = await getText(loginUrl);
    return loginInfoText2Object(loginInfoText);
}

function loginInfoText2Object(text) {
    // window.code=408
    // window.code=201;window.userAvatar = 'data:img/jpg;base64,';
    // 正则表达式不是太熟练，换个简单但是不是太安全的方法
    const preText = text.replace(/window\./g, 'this.');
    const result = new(function() {
        eval(preText);
    })();
    return result;
}

function getLoginQRCode(uuid) {
    return `https://login.weixin.qq.com/qrcode/${uuid}`;
}

async function test() {
    await get('https://wx.qq.com/');
    const uuid = await getUUID();
    console.log(getLoginQRCode(uuid));
    setInterval(async () => console.log(await cheakLoginInfo(uuid)), 8000);

}

test();